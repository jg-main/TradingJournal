 ...
 *
 * Run: npx tsx src/app/api/trades/export/__tests__/route.test.ts
 */

process.env.DB_FILE_NAME = './.test-m06-s01-t02-db';
...
const DB_FILE = process.env.DB_FILE_NAME || './.test-m06-s01-t02-db';
  ...
  try {
    ...
    const exportRows: ExportTradeRow[] = allTrades.map((trade) => {
      ...
      return {
        ...
        plannedTarget2: trade.plannedTarget2 ?? null,
...
function seedTrade(
  ...
  db.insert(schema.trades).values({
    ...
    plannedTarget2: overrides?.plannedTarget2 ?? null,
