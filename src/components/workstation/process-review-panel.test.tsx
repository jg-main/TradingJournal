 ...
 * It is read-only in both live and fixture mode — it renders the shared
 * snapshot and never fetches on its own.
 *
 * These tests pin:
 *
 ...
 *   - the panel never fetches (no self-owned data requests)
 *
...
describe('ProcessReviewPanel', () => {
  ...
  it('never fetches — the panel is a read-only consumer of the shared snapshot', () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation(() => {
      throw new Error('ProcessReviewPanel must not fetch');
    });
