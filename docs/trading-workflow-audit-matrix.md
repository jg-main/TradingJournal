...
| Status guard | **M002-A12: the canonical `executeTradeFill` enforces the full lifecycle gate**: planned → first-fill gates (readiness/checklist/max-risk); open → add/reduce/close per direction semantics; **closed → every NEW ordinary execution is rejected pre-flight with typed `TradeClosedError` → 409 `TRADE_CLOSED_EXECUTION_REJECTED` (zero mutation — no journal/accounting/events/ledger/FIFO/performance writes, no `updatedAt` stamp, no risk-snapshot/checklist/review touches)**; deleted → `TradeDeletedError` (distinct 400). Guard ordering is load → deleted → **SAME-key idempotency replay** → closed → action/quantity validation, so a network retry of the final closing fill replays (same execution id, trade stays closed) and a NEW key after close is rejected. Both routes map the typed error identically; the `/execute` bulk adapter fails on the FIRST attempted fill (zero fills) while same-base-key retries of a completed closing bulk replay. Open quantity for the over-close preflight and the post-insert status derivation are **correction-aware** (accounting stream + lineage), so a correction-reopened trade accepts normal management fills again. The ONLY mechanism that alters a closed trade's execution stream is execution correction — which may legitimately reopen (remaining quantity > 0 → open, closedAt null, reviewedAt cleared) or reclose. No status-mutation reopening endpoint exists. | **Current** (A12) | §17: a closed trade with a new request is rejected as closed (never a confusing over-close); the immutable execution-correction workflow is the sole historical-alteration path. |
...
|---|---|---|---|
| Trade creation | `POST /api/trades` — validates planned-stop geometry via canonical `computePlannedRiskAmount` (R025, only when both entry+stop supplied; partial combos skip); account resolution is M002-A10/A11: **explicit `accountId` → use exactly that account or reject (404 `ACCOUNT_NOT_FOUND` / 409 planning-eligibility); omitted → eligible saved default (exists + active + USD, per M006-A8 `isAccountEligibleAsDefault`) → first planning-eligible active USD account ordered by createdAt ASC → 400 if none.** The old execution-readiness ranking step (max_risk/commission/opening-cash) is REMOVED — automatic planning considers planning eligibility only; execution readiness is the first-fill boundary. `tradeCode` `T-XXXX` generated with 3-attempt UNIQUE retry; **status hardcoded `'planned'`** at insert. | **Current** (A10/A11) | Planning eligibility is lighter (exists/active/USD); a plan-only account may own an automatically created planned trade. M002-A1: execution readiness resolves effective risk/commission through account override → global default → unavailable at first fill only. |
...
| Idempotency | **M002-A13: journal execution idempotency keys are globally unique (`uq_trade_executions_idempotency_key` partial unique index, migration 0037 — unchanged) and permanently owned by exactly one journal trade.** `executeTradeFill` preflight: load trade → deleted guard → key lookup — same key + same trade → replay (reconstructed from the PERSISTED execution's own trade identity via `buildReplayResult`, so execution/trade/risk identities can never mix; `execution.tradeId == trade.id` and accounting linkage match); same key + ANOTHER trade → typed `ExecutionIdempotencyConflictError` → 409 `EXECUTION_IDEMPOTENCY_CONFLICT` (zero mutation, read-only preflight). The concurrent UNIQUE-violation path enforces the same ownership rule (foreign collision → conflict, never replay). The `/execute` bulk adapter ownership-preflights EVERY derived key (`K:entry`/`K:exit1`/`K:exit2`) before the first fill, so a partial foreign collision rejects the whole request with zero fills, while same-base-key same-trade retries replay. A12 ordering preserved: closed trade + own final-close key → replay; closed + fresh key → `TRADE_CLOSED_EXECUTION_REJECTED`; closed + foreign key → conflict. UI: `AddFillDialog` mints ONE stable client key per logical submission and reuses it across retries (cleared on success/close) — a network retry cannot duplicate a fill. Server-minted fallback keys guarantee uniqueness of the accepted call only (documented; NOT network-retry-safe). Historical rows need no backfill — a persisted key automatically owns its trade. | **Current** (A13) | §10 contract met: client-generated key accepted, reused on retry, not consumed on failed transaction; cross-trade reuse is a deterministic 409 conflict; correction idempotency (own lineage machinery) and `POST /api/accounts/:id/executions` (separate accounting boundary) are untouched. |
