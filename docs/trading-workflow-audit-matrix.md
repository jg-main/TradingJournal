...
| Manual risk-snapshot PUT surface | `PUT /api/trades/[id]/risk-snapshot` accepts **client-supplied** `accountEquityAtOpen`, `initialEntryPrice`, `initialStopPrice`, `initialRiskAmount`, etc. (upsert-or-patch). T03-verified: **no active UI consumer** — `handleRiskSnapshotSave` in `src/app/(legacy)/trades/[id]/page.tsx` L447 is dead code (never threaded to a component; `RiskSnapshotCard` is read-only). | **Replace/Refine** | §16 violation surface: anyone with the API can rewrite the deterministic first-entry snapshot (incl. equity and risk amount) after the fact, corrupting R-multiple/riskToAccount history. Remove, or restrict to an audited correction-driven rebuild (S06). |
...
| L3 | Manual snapshot PUT | `PUT /api/trades/[id]/risk-snapshot` accepts arbitrary client snapshot values; **dead UI code** (`handleRiskSnapshotSave` never wired; `RiskSnapshotCard` read-only). | **Replace/Refine** |
